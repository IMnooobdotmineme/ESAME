...
export default function ScorePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await fetch(`/api/student/result?roomCode=${encodeURIComponent(s.roomCode)}&requestId=${encodeURIComponent(s.requestId)}`);
      if (!res.ok) { router.replace("/join"); return; }
      const data = await res.json();
      setExamTitle(data.examTitle);
      setScore({
        totalQuestions: data.totalQuestions,
        autoGradedQuestions: data.autoGradedQuestions,
        correctCount: data.correctCount,
        incorrectCount: data.incorrectCount,
        manualGradeCount: data.manualCount,
        unansweredCount: data.unansweredCount,
        percentage: data.percentage,
        earnedPoints: data.earnedPoints,
        autoTotalPoints: data.autoTotalPoints,
        totalPoints: data.autoTotalPoints,
      });
      setSections(data.sections);
      setResult({ answers: {}, hasEssay: data.hasManual, submittedAt: data.submittedAt, reason: undefined });
