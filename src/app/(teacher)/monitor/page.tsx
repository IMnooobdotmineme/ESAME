...
import {
  Radio, Users, CheckCircle2, AlertTriangle, Copy, Check, Square,
  Clock, ShieldAlert, Key, Pause, Play, FileCheck2, ClipboardList,
  UserX, MessageSquare,
} from "lucide-react";
...
export default function TeacherLiveMonitorPage() {
  ...
  const activeExam = exams.find((e) => e.id === pinnedExamId) || liveExam || launchedNotStarted;
  ...
  if (!activeExam) {
    return (
      <>
        <TeacherTopbar title="Live Monitoring" description="Real-time candidate monitoring." />
        <main className="min-h-[60vh] flex items-center justify-center p-6">
          <Card className="max-w-md w-full p-8 text-center space-y-4">
            ...
            <h2 className="text-lg font-bold text-navy-900">No Active Examination</h2>
  ...
  if (!activeExam.isStarted && !activeExam.isEnded) {
    return (
      <>
        <TeacherTopbar title={activeExam.title} description="Exam not started yet." />
        <main className="min-h-[60vh] flex items-center justify-center p-6">
          <Card className="max-w-md w-full p-8 text-center space-y-4">
            ...
            <h2 className="text-lg font-bold text-navy-900">Exam Not Started Yet</h2>
            <p className="text-xs font-medium text-slate-500">
            ...
            <Button className="w-full" onClick={() => router.push(`/teacher-exams/${activeExam.roomCode}`)}>
              Go to Lobby
            </Button>
          </Card>
        </main>
  ...
  const handlePauseResume = async () => {
    ...
    if (activeExam.isPaused) await resumeExam(activeExam.roomCode);
    else await pauseExam(activeExam.roomCode);
    setLoadingAction(null);
  };
  ...
  if (activeExam.isEnded) {
    return (
      <>
        <TeacherTopbar title={activeExam.title} description="This session has ended." />
          ...
          <Card className="p-8 text-center space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3 pt-2 max-w-md mx-auto">
              ...
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-4">
                ...
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-1">Finished</p>
              </div>
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-4">
            ...
            <div className="flex items-center justify-center gap-3 pt-2">
              <Button variant="outline" onClick={() => router.push("/teacher-exams")}>Back to My Exams</Button>
              <Button onClick={() => router.push("/grading")}>
                <ClipboardList className="w-4 h-4" /> Go to Grading
              </Button>
            </div>
          </Card>
        </main>
  ...
  return (
    <>
      <TeacherTopbar title={activeExam.title || "Live Monitoring"} description="Real-time candidate monitoring." />
      <main className="w-full max-w-6xl mx-auto p-6 space-y-6">
        {/* ACTION ROW */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
          <div className="flex items-center gap-2 text-slate-500 text-xs font-semibold uppercase tracking-wider">
            <span className={`w-2 h-2 rounded-full ${activeExam.isPaused ? "bg-amber-500" : "bg-emerald-500 animate-pulse"}`} />
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="bg-white border border-slate-200 px-3 py-1.5 rounded-xl flex items-center gap-2">
              ...
              <button onClick={() => handleCopyCode(activeExam.roomCode)} className="text-slate-400 hover:text-slate-600 p-0.5">
            ...
            <Button variant="outline" size="sm" onClick={handlePauseResume} disabled={loadingAction === "pause"}>
              {loadingAction === "pause" ? "..." : activeExam.isPaused ? <><Play className="w-3.5 h-3.5 mr-1" /> Resume</> : <><Pause className="w-3.5 h-3.5 mr-1" /> Pause</>}
            </Button>
            <Button variant="danger" size="sm" onClick={handleStopExam} disabled={loadingAction === "end"}>
              {loadingAction === "end" ? "Ending..." : <><Square className="w-3.5 h-3.5 fill-current mr-1" /> End Session</>}
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Participants</p>
              <p className="text-2xl font-bold text-navy-900">{counts.all}</p>
            </div>
            <div className="p-2.5 rounded-xl bg-slate-50 text-slate-600 border border-slate-200"><Users className="w-5 h-5" /></div>
          </Card>
          <Card className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-700">Finished</p>
              <p className="text-2xl font-bold text-navy-900">{counts.finished}</p>
            </div>
            <div className="p-2.5 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200"><CheckCircle2 className="w-5 h-5" /></div>
          </Card>
          <Card className={`p-5 flex items-center justify-between ${counts.attention > 0 ? "border-rose-200 bg-rose-50/50" : ""}`}>
            <div className="space-y-1">
              <p className={`text-[10px] font-bold uppercase tracking-wider ${counts.attention > 0 ? "text-rose-800" : "text-slate-400"}`}>Requires Attention</p>
              <p className={`text-2xl font-bold ${counts.attention > 0 ? "text-rose-800" : "text-navy-900"}`}>{counts.attention}</p>
            </div>
            <div className={`p-2.5 rounded-xl ${counts.attention > 0 ? "bg-rose-100 text-rose-800 border border-rose-200" : "bg-slate-50 text-slate-400 border border-slate-200"}`}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {visibleStudents.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-10 text-center text-slate-400 text-sm">No students in this category right now.</td></tr>
              ) : (
                visibleStudents.map((student) => {
                  const isActionLoading = loadingAction === student.id;
                  return (
                    <tr key={student.id} className={`border-b border-slate-50 last:border-0 ${student.isLocked ? "bg-rose-50/40" : "hover:bg-slate-50/50"}`}>
                      ...
                      <td className="px-5 py-3.5">
                        {student.isRejectedLive ? (
                          ...
                          <Badge variant="success">{student.isForcedSubmit ? "Force Submitted" : "Finished"}</Badge>
                        ) : (
                      ...
                      <td className="px-5 py-3.5 text-right space-x-2">
                        {student.isRejectedLive || student.isSubmitted || student.isForcedSubmit ? (
                          ...
                          <>
                            <button onClick={() => setViewingMessageOf(student)} title="View message"
                              className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-slate-200 text-slate-500 hover:text-navy-900 hover:bg-slate-50 align-middle">
                              <MessageSquare size={14} />
                            </button>
                            <Button size="sm" variant="danger" disabled={isActionLoading} onClick={() => handleRejectLive(student)}>
                              {isActionLoading ? "..." : <><UserX size={13} /> Reject</>}
                            </Button>
                            <Button size="sm" disabled={isActionLoading} onClick={() => handleGrantContinue(student)}>
                              {isActionLoading ? "..." : "Approve to Continue"}
                            </Button>
                          </>
                        ) : (
                          <Button size="sm" variant="danger" disabled={isActionLoading} onClick={() => handleRejectLive(student)}>
                            {isActionLoading ? "..." : <><UserX size={13} /> Reject & Force Submit</>}
                          </Button>
                        )}
                      </td>
                    </tr>
      ...
      <Dialog open={!!viewingMessageOf} onClose={() => setViewingMessageOf(null)}>
        {viewingMessageOf && (
          <>
            <DialogHeader title={`${viewingMessageOf.name} (${viewingMessageOf.studentId})`} onClose={() => setViewingMessageOf(null)} />
