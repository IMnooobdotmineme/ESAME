...
export default function TeacherExamLobbyPage() {
      ...
      <>
        ...
        <main className="min-h-[70vh] flex items-center justify-center p-6">
          <Card className="max-w-md w-full p-8 text-center space-y-4">
            ...
            <p className="text-sm text-slate-500">The exam room code could not be located.</p>
            <Button className="w-full" onClick={() => router.push("/teacher-exams")}>Back to Exams List</Button>
          </Card>
        </main>
      </>
    );
  ...
  const handleStartExam = async () => {
    setStarting(true);
    const result = await startExam(exam.roomCode);
    setStarting(false);
    if (!result.success) {
      ...
      <main className="p-6 space-y-6">
        <div className="flex items-center justify-between border-b border-slate-200 pb-4">
          ...
          {exam.isStarted && !exam.isEnded ? (
            <button onClick={() => router.push("/monitor")} className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-bold hover:bg-emerald-100">
        ...
        <Card className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            {!exam.isStarted && !exam.isEnded && (
              <div className="pt-2">
                <Button onClick={handleStartExam} disabled={starting || approvedStudents.length === 0}>
                  {starting ? "Starting..." : <><Play className="w-4 h-4 fill-current mr-1" /> Start Exam Now ({approvedStudents.length} Approved)</>}
                </Button>
                {approvedStudents.length === 0 && (
                  <p className="text-xs text-amber-600 mt-2">Approve at least one student first.</p>
                )}
              </div>
            )}
          </div>
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 text-center min-w-[220px]">
            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Student Access Code</p>
            <p className="text-2xl font-mono font-extrabold text-navy-900 tracking-widest mt-1">{exam.roomCode}</p>
            <p className="text-[11px] text-slate-400 mt-1 font-medium">Share this code for student access</p>
          </div>
        </Card>
