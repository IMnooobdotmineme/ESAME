...
function AcceptInviteForm() {
  ...
  useEffect(() => {
    if (!token) {
      setError("This invitation link is missing a valid token.");
    }
  }, [token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess("Your teacher account is ready. Redirecting to login...");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#F0F3FA] via-white to-[#D5DEEF] py-12 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-[480px] bg-white p-8 sm:p-10 rounded-3xl shadow-xl shadow-[#395886]/10 border border-[#D5DEEF]/60">
        <div className="inline-flex items-center text-sm font-semibold text-[#1F2A44] mb-6">
          <span className="w-10 h-10 rounded-2xl bg-[#F0F3FA] flex items-center justify-center mr-3">
            <svg className="w-5 h-5 text-[#395886]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
        ...
        <div className="mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#395886] tracking-tight">Create your password</h2>
          <p className="mt-2 text-sm text-[#638ECB] leading-relaxed">
            Finish your invitation by creating a secure password for your teacher account.
          </p>
        </div>
          ...
          <div>
            ...
            <div className="relative mt-2">
              ...
              <button
                ...
                className="absolute inset-y-0 right-0 pr-4 flex items-center text-[#8AAEE0] hover:text-[#395886]"
              >
                {showPassword ? (
...
export default function AcceptInvitePage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-[#f5f7fb] text-[#395886] font-medium">Loading invitation...</div>}>
      <AcceptInviteForm />
    </Suspense>
  );
}
