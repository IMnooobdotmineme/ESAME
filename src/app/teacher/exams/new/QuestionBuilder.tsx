...
export default function QuestionBuilder() {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden max-w-4xl mx-auto text-slate-900">
      ...
      <div className="bg-slate-50 border-b border-slate-200 p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h4 className="text-lg font-bold text-slate-900">Dynamic Question Workspace</h4>
          <p className="text-xs text-slate-500">Configure parameters, points metrics, and evaluation conditions.</p>
        </div>
        
        {/* Core Question Type Select Engine (The exam.net magic) */}
        <div className="w-full sm:w-auto">
          <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Question Type</label>
          <select 
            value={selectedType}
            onChange={(e) => setSelectedType(e.target.value as QuestionType)}
            className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm font-semibold shadow-sm focus:border-[#0B7A93] focus:ring-1 focus:ring-[#0B7A93] outline-none transition-all"
          >
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Score Weight (Points)</label>
            <input 
              type="number" 
              min="1"
              value={meta.points}
              onChange={(e) => setMeta({...meta, points: parseInt(e.target.value) || 1})}
              className="w-full border border-slate-300 rounded-lg px-3 py-1.5 text-sm font-medium"
            />
          </div>
          <div className="flex items-center pt-5">
            <label className="flex items-center gap-2 cursor-pointer text-sm font-bold text-slate-700">
              <input 
                ...
                className="w-4 h-4 rounded text-[#0B7A93] focus:ring-[#0B7A93] border-slate-300"
              />
              Mandatory Question
            </label>
          </div>
        ...
        <div>
          <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Question Prompt / Prompt Body</label>
          <textarea 
            rows={3}
            placeholder="Enter the query or text prompt instructions for the examinee here..."
            value={questionText}
            onChange={(e) => setQuestionText(e.target.value)}
            className="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:border-[#0B7A93] focus:ring-1 focus:ring-[#0B7A93] outline-none"
          />
        </div>
        ...
        <div className="border-t border-dashed border-slate-200 pt-6">
          ...
          {selectedType === 'mcq' && (
            <div className="space-y-3">
              <label className="block text-xs font-bold text-slate-500 uppercase">Answer Options & Correct Target</label>
          ...
          {selectedType === 'true_false' && (
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Target Truth Assignment</label>
          ...
          {selectedType === 'short_answer' && (
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-500 uppercase">Accepted Text Matches (Case Insensitive)</label>
              <input 
                type="text"
          ...
          {selectedType === 'essay' && (
            <div className="grid grid-cols-2 gap-4 max-w-md">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Minimum Word Bounds</label>
                <input 
                  type="number"
                  value={wordLimit.min}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Maximum Word Bounds</label>
                <input 
                  type="number"
                  value={wordLimit.max}
          ...
          {selectedType === 'coding' && (
            <div className="space-y-4">
              <div className="max-w-xs">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Runtime Compiler Sandbox</label>
                <select 
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Boilerplate / Starter Code Given to Student</label>
                <textarea 
                  rows={4}
        ...
        <div className="border-t border-slate-100 pt-4">
          <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Post-Grading Explanatory Feedback Notes</label>
          <input 
            type="text"
            ...
            className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-slate-50/30"
          />
        </div>
